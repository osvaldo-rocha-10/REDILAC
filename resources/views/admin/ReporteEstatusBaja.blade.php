...
@section('content')
      <div class="breadcrumb-holder">
        <div class="container-fluid">
          <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item active">SUBSECCIÓN REPORTES CON ESTATUS BAJA</li>
          </ul>
        </div>
     </div>
      <div class="card-body">
       ...
       <form method="post" action="{{{route('reporte.generar',['estatus' => 0,'tipo' => 1])}}}">
          @csrf
         <div class="container-fluid">
          <div class="card shadow mb-4">
                <div class="card income"  style="border-color: #003b5c;  border-width: 2px; padding: 10px; margin: 3px;">
                     <center> 
                      <h6 class="m-0 font-weight-bold text-primary"> 4.3 REPORTES CON ESTATUS BAJA.</h6>
                     </center> <br>
                          ...
                          <div class="col-md-12">
                            <center>  
                              <h6 class="m-0 font-weight-bold text-primary">
                                   ...
                                   4.3.1.- Seleccione filtro por "Dirección/Academia":
                                     ...
                                     4.3.2.-(Opcional) Seleccione filtro por "Categoria de  Equipo".
                                     <select name="categoria" id="categoria">
                                            <option value="">SN-CATEGORIA</option>
                              
                                      </select> <br> <br>
                                     4.3.3.- Para realizar una busqueda mas profunda, busque de manera automatica por NoInventario al momento de seleccionar el filtro "Dirrección-Academia y/o Categoria de Equipo"<br> <br>

                                     4.3.4.-Descargue en formatos PDF y EXCEL, ademas de guardar e imprimir reportes desde el sistema.
                                </h6>
                            </center>
                          </div> 
       ...
       $("#academia").change(function(){
            ...
            if($('select[id=academia]').val()!=''){
               ...
               var _token = $('input[name="_token"]').val(); 
                 ...
                 $.ajax({
                   url:"{{ route('equipo.combobox_ac',['estatus'=> 0])}}",
                    method: "POST",
                ...
                $.ajax({
                    ...
                    url:"{{ route('equipo.tabla_a',['opc' => 2,'estatus' => 0])}}",
                    method: "POST",
                    data:{value:value,_token:_token},
                    ...
                    success:function(result){
                       $('#equipo_academia').html(result);
                        if(result!=''){
                          $('.informacion').show();
                          $('#mensaje').hide();
                          $('#titulo').html(" <div class='alert alert-success'role='alert'>ACADEMIA:"+academia+"/ESTATUS:BAJA</div>");
                       }
                       else {
                          $('.informacion').hide();
                           $('#mensaje').show();
                           $('#mensaje').html('NO HAY EQUIPOS  CON ESTATUS BAJA EN -"DIRECCIÓN/ACADEMIA: '+academia+'"-  EN ESTE MOMENTO.');    
                       }
    ...
    $("#categoria").change(function(){
        if($('select[id=categoria]').val()!=''){
              ...
              categoria = $(this).find(':selected').data('categoria');
                 ...
                 $.ajax({
                    url:"{{ route('equipo.tabla_ac',['opc' => 2,'estatus'=> 0])}}",
                    method: "POST",
                    ...
                    success:function(result){
                      ...
                      $('#equipo_academia').html(result);
                      if(result!='')
                           $('#titulo').html(" <div class='alert alert-success'role='alert'>ACADEMIA:"+academia+"/"+categoria+" /ESTATUS:BAJA</div>");
   ...
   $(document).ready(function(){
       ...
       $("#busqueda").keyup(function(){
          ...
          if(value_categoria==''){
            $.ajax({
                    url:"{{route('equipo.tabla_ab',['opc' => 2, 'estatus' => 0])}}", 
                    method: "POST",
              ...
              $.ajax({
                    url:"{{route('equipo.tabla_abc',['opc' => 2, 'estatus' => 0])}}", 
                    method: "POST",
