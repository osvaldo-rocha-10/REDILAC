...
@section('content')
      ...
      <div class="card-body">
       ...
       <form method="post" action="{{{route('reporte.generar',['estatus' => 0,'tipo' => 2])}}}">
          @csrf
         <div class="container-fluid">
          <div class="card shadow mb-4">
                <div class="card income"  style="border-color: #003b5c;  border-width: 2px; padding: 10px; margin: 3px;">
                     <center> 
                      <h6 class="m-0 font-weight-bold text-primary"> 3.3 REPORTES CON ESTATUS BAJA.</h6>
                     </center> <br>
                          ...
                          <div class="col-md-12">
                            <center>  
                              <h6 class="m-0 font-weight-bold text-primary">
                                   ...
                                   3.3.1.- Seleccione filtro por "Instalación":
                                    ...
                                    3.3.3.- Para realizar una busqueda mas profunda, busque de manera automatica por NoInventario al momento de seleccionar el filtro "Instalación y/o Categoria de Equipo"<br> <br>

                                    3.3.4.-Descargue en formatos PDF y EXCEL, ademas de guardar e imprimir reportes desde el sistema.
                                </h6>
                            </center>
                          </div> 
         ...
         $("#instalacion").change(function(){
            ...
            if($('select[id=instalacion]').val()!=''){
               ...
               var _token = $('input[name="_token"]').val(); 
                 ...
                 $.ajax({
                    url:"{{route('equipo.combobox_ic',['estatus' => 0])}}", 
                    method: "POST",
                ...
                $.ajax({
                    url:"{{route('equipo.tabla_i',['opc' => 2,'estatus' => 0])}}",      
                    method: "POST",
                    data:{instalacion:instalacion,_token:_token},
                    success:function(result){
                        ...
                        if(result!=''){
                          ...
                          $('.informacion').hide(); 
                          $('#mensaje').show();
                          $('#mensaje').html('NO HAY EQUIPOS  CON ESTATUS BAJA EN -"INSTALACIÓN: '+nomenclatura+'"-  EN ESTE MOMENTO.'); 
                       }
                    }
               })
     ...
     $("#categoria").change(function(){
        if($('select[id=categoria]').val()!=''){
             ...
             $.ajax({
                    url:"{{route('equipo.tabla_ic',['opc' => 2,'estatus' => 0])}}",  
                    method: "POST",
                    ...
                    success:function(result){
                          ...
                          $('#equipo_instalacion').html(result);
                          if(result!=''){
                            $('#titulo').html(" <div class='alert alert-success'role='alert'> INSTALACIÓN: "+nomenclatura+" / "+tipo+" /ESTATUS:BAJA</div>");
                          }
   ...
   $(document).ready(function(){
       ...
       $("#busqueda").keyup(function(){
          ...
          if(categoria==''){
            $.ajax({
                    url:"{{route('equipo.tabla_ib',['opc' => 2,'estatus' => 0])}}", 
                    method: "POST",
              ...
              $.ajax({
                    url:"{{route('equipo.tabla_ibc',['opc' => 2,'estatus' => 0])}}", 
                    method: "POST",
